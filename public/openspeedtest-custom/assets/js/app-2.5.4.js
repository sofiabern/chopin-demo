...
window.onload = function() {
  ...
  // CHOPIN DEMO: Add postMessage listener for ping-pong communication
  window.addEventListener('message', function(event) {
    if (event.source === window.parent && event.data && event.data.message === 'ping') {
      console.log('OpenSpeedTest: Received ping, sending pong.');
      window.parent.postMessage({ message: 'pong' }, '*');
    }
  });
      ...
      var Engine = setInterval(function() {
        ...
        if (Status === "Uploading") {
          ...
          if (uploadTimeing >= ulDuration && stop == 1) {
            ...
            // CHOPIN DEMO: Notify parent that test is complete with results
            if (window.parent && window.parent !== window) {
              console.log('OpenSpeedTest: Test completed, sending results to parent.');
              setTimeout(function() {
                var downloadResult = document.getElementById('downResult');
                var uploadResult = document.getElementById('upRestxt');
                var pingResult = document.getElementById('pingResult');
                
                if (downloadResult && uploadResult && pingResult) {
                  var payload = {
                    message: 'test_completed',
                    results: {
                      download: parseFloat(downloadResult.textContent || downloadResult.innerText),
                      upload: parseFloat(uploadResult.textContent || uploadResult.innerText),
                      ping: parseFloat(pingResult.textContent || pingResult.innerText)
                    }
                  };
                  console.log('OpenSpeedTest: Sending results:', payload);
                  window.parent.postMessage(payload, '*');
                } else {
                  console.log('OpenSpeedTest: Could not find result elements');
                }
      ...
      if (pingSendStatus < pingSendLength - 1) {
        ...
        if (pingServer.length >= 1) {
          ...
          if (SelectTest) {
            ...
            Status = "Download";
            // CHOPIN DEMO: Notify parent that test has started
            if (window.parent && window.parent !== window) {
              console.log('OpenSpeedTest: Test started, notifying parent.');
              window.parent.postMessage({ message: 'test_started' }, '*');
            }
          }
        } else {
